...
const train = () => {
  ...
  // Add more training data for improving brand with examples
  classifier.addDocument(
    "How can I improve my brand with X examples?",
    "improve_brand_examples"
  );
...
const run = () => {
  ...
  switch (label) {
    ...
    case "improve_brand":
      ...
      break;
    case "improve_brand_examples":
      // Use a regular expression to match and extract the number from the input
      const numberMatch = input.match(/(\d+)/);
      const numberOfExamples = numberMatch ? parseInt(numberMatch[0]) : 1;

      response = `To improve your brand with ${numberOfExamples} example(s), analyze successful brands and identify their key strategies. Implement these strategies in your own brand development, such as creating a unique visual identity, building a strong online presence, and engaging with your audience.`;
      break;
    case "improve_impact":
      response =
